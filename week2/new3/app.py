...
class File(db.Model):
    ...
    def add_tag(self, tag_name):
        file_tag = db1.file.find_one({'file_id': self.id})
        print(file_tag)
        if file_tag:
            tags = file_tag['tags']
            print(tags)
            ...
            db1.file.update_one({'file_id': self.id}, {'$set': {'tags': tags}})
        else:
            tags=[tag_name]
            db1.file.insert_one({'file_id': self.id},{'tags': tags})
        return  tags
    ...
    @property
    def tags(self):
        file_tag = db1.file.find_one({'file_id': self_id})
        if file_tag:
...
def file(file_id):
    ...
    return rendet_template('file.html', file_id=file_id)
